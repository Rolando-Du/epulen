...
const Dashboard = () => {
  ...
  const totalEquipos = productos.reduce(
    (acc, p) =>
      acc +
      (p.tallas?.reduce((a, t) => a + Number(t.stock || 0), 0) || 0),
    0
  );
  ...
  const generarReportePDF = () => {
    try {
      ...
      doc.text(`Reporte de inventario · ${fecha}`, 14, 24);
      ...
      productos.forEach((p) => {
        ...
        tallas.forEach((t) => {
          ...
          tablaDatos.push([
            ...
            `$${(stock * precio).toLocaleString("es-AR")}`,
          ]);
        });
      });
      ...
      const resumenY = finalY > 270 ? 20 : finalY + 12;
      ...
      doc.save(`Inventario_Epulen_${fecha.replace(/\//g, "-")}.pdf`);
  ...
  const agregarTalleALista = () => {
    ...
    const talleFormateado = `T-${valorLimpio}`;

    const existe = producto.tallas.find(
      (t) => t.talle === talleFormateado
    );
    ...
    if (existe) {
      Swal.fire({
        ...
        text: "Ese talle ya fue agregado al producto.",
      });
      return;
    }
    ...
    setProducto({
      ...
      tallas: [
        ...producto.tallas,
        {
          talle: talleFormateado,
          stock: Number(inputCantidad),
        },
      ],
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (producto.tallas.length === 0) {
      Swal.fire({
        ...
        icon: "error",
        title: "Faltan talles",
        text: "Agregá al menos un talle con stock.",
      });
      return;
    }
    ...
    Swal.fire({
      ...swalEstilo,
      title: editando ? "Actualizando producto..." : "Creando producto...",
        ...
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8 mb-10 pb-8 border-b border-[#D8DDD4]">
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4 items-stretch sm:items-center">
            ...
            <div className="min-w-32.5 bg-[#FCFBF8] border border-[#D8DDD4] rounded-xl px-4 py-3">
              <p className="text-[11px] text-[#899189]">Stock total</p>
            ...
            <div className="flex items-start justify-between gap-4 mb-7">
              <div>
                <p className="text-xs text-[#788873] font-medium mb-1">
                  {editando ? "Edición" : "Nuevo producto"}
                </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="bg-[#F7F6F2] rounded-3xl min-h-52.5 p-5 border border-[#E0E3DD] flex items-center justify-center">
                {imagenesFiles.length > 0 ? (
                  ...
                  <img
                    src={resolverImagen(producto.imagenUrlPrevia)}
                    className="h-44 max-w-full object-contain rounded-xl"
                    alt="Imagen actual"
                  />
                ) : (
              ...
              <div className="bg-[#F3F5F0] p-4 sm:p-5 rounded-2xl border border-[#DCE1D8]">
                <div className="mb-4">
                  ...
                  <p className="text-xs text-[#7D867E] mt-1">
                    Agregá cada talle con su cantidad disponible.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {producto.tallas.length === 0 ? (
                    ...
                    producto.tallas.map((t, i) => (
                      ...
                      >
                        <strong className="font-medium">{t.talle}</strong>
                        <span className="text-[#8B948C]">{t.stock} uds.</span>
